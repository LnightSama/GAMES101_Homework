                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
	...
	//invDir：光线方向（x，y，z），invDir=（1.0/x，1.0/y，1.0/z），使用此选项是因为乘法比除法更快
    //dirIsNeg：光线方向（x，y，z），dirIsNeg=[int（x>0），int（y>0），int（z>0）]，使用此选项可以简化逻辑
    //测试光线边界是否相交

    //x
	//float ox = ray.origin.x, oy = ray.origin.y, oz = ray.origin.z;
	//float dx = invDir.x, dy = invDir.y, dz = invDir.z;
	//float list_t[6] = {
	//	(pMax.x - ox) * dx,
	//	(pMin.x - ox) * dx,
	//	(pMax.y - oy) * dy,
	...
	//	}
	//}
	//if (list_t[3] >= 0 && list_t[2] < list_t[3])
	//{
	//	return true;
	//}
	//return false;
