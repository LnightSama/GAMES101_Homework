...
Vector3f Scene::castRay(const Ray &ray, int depth) const
{
	...
	if (!inter.happened) //没交点
		return {};
	if (inter.m->hasEmission()) //一、交点是光源：
    ...
    {
        L_Dir =light_inter.emit * inter.m->eval(ray.direction, point_light_dir, inter.normal) * dotProduct(point_light_dir, inter.normal) * dotProduct(- point_light_dir, inter.normal) / dotProduct(light_point_v3, light_point_v3) /
            pdf_light;
    }
